...
def support_jsonp(f):
    ...
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location', False)
        if callback:
...
def support_jsonp_geo_click_region(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_geo_click_region', False)
        if callback:
            content = str(callback) + '(' + str(f().data) + ')'
            return current_app.response_class(content, mimetype='application/json')
        else:
...
def support_jsonp_curriculum(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_curriculum', False)
        if callback:
            content = str(callback) + '(' + str(f().data) + ')'
            return current_app.response_class(content, mimetype='application/json')
        else:
...
def support_jsonp_student(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_student', False)
        if callback:
            content = str(callback) + '(' + str(f().data) + ')'
            return current_app.response_class(content, mimetype='application/json')
        else:
...
def support_jsonp_discovery(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_discovery', False)
        if callback:
            content = str(callback) + '(' + str(f().data) + ')'
            return current_app.response_class(content, mimetype='application/json')
        else:
...
def support_jsonp_exploration(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_exploration', False)
        if callback:
            content = str(callback) + '(' + str(f().data) + ')'
            return current_app.response_class(content, mimetype='application/json')
        else:
...
def support_jsonp_switching_routing(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_exploration', False)
...
def support_jsonp_rank_institution(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_rank_institution', False)
        if callback:
            content = str(callback) + '(' + str(f().data) + ')'
            return current_app.response_class(content, mimetype='application/json')
        else:
...
def support_jsonp_geo_loc_institutions(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_geo_loc_institutions', False)
        if callback:
            content = str(callback) + '(' + str(f().data) + ')'
            return current_app.response_class(content, mimetype='application/json')
        else:
...
def support_jsonp_geo_loc_students(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_geo_loc_students', False)
        if callback:
            content = str(callback) + '(' + str(f().data) + ')'
            return current_app.response_class(content, mimetype='application/json')
        else:
...
def support_jsonp_gender(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_gender', False)
        if callback:
            content = str(callback) + '(' + str(f().data) + ')'
            return current_app.response_class(content, mimetype='application/json')
        else:
...
def support_jsonp_curriculum_trending(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_curriculum_trending', False)
        if callback:
            content = str(callback) + '(' + str(f().data) + ')'
            return current_app.response_class(content, mimetype='application/json')
        else:
...
def support_jsonp_gender_trending(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_gender_trending', False)
        if callback:
            content = str(callback) + '(' + str(f().data) + ')'
            return current_app.response_class(content, mimetype='application/json')
        else:
...
def support_jsonp_split_curriculum_trending(f):
    """Wraps JSONified output for JSONP"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        callback = request.args.get('get_location_split_curriculum_trending', False)
        if callback:
            content = str(callback) + '(' + str(f().data) + ')'
            return current_app.response_class(content, mimetype='application/json')
        else:
...
class CurrentDiscovery():

    #Local variables
    nb_student_lvl_1_ccna_discovery=0
    nb_student_lvl_2_ccna_discovery=0
    nb_student_lvl_3_ccna_discovery=0
    nb_student_lvl_4_ccna_discovery=0


    def __init__(self, nb_student_lvl_1_ccna_discovery, nb_student_lvl_2_ccna_discovery, nb_student_lvl_3_ccna_discovery, nb_student_lvl_4_ccna_discovery) :
        self.nb_student_lvl_1_ccna_discovery=nb_student_lvl_1_ccna_discovery
        self.nb_student_lvl_2_ccna_discovery=nb_student_lvl_2_ccna_discovery
        self.nb_student_lvl_3_ccna_discovery=nb_student_lvl_3_ccna_discovery
        self.nb_student_lvl_4_ccna_discovery=nb_student_lvl_4_ccna_discovery


    def add_som_discovery(self, nb_student_lvl_1_ccna_discovery, nb_student_lvl_2_ccna_discovery, nb_student_lvl_3_ccna_discovery, nb_student_lvl_4_ccna_discovery):
        self.nb_student_lvl_1_ccna_discovery=self.nb_student_lvl_1_ccna_discovery + nb_student_lvl_1_ccna_discovery
        self.nb_student_lvl_2_ccna_discovery=self.nb_student_lvl_2_ccna_discovery + nb_student_lvl_2_ccna_discovery
        self.nb_student_lvl_3_ccna_discovery=self.nb_student_lvl_3_ccna_discovery + nb_student_lvl_3_ccna_discovery
        self.nb_student_lvl_4_ccna_discovery=self.nb_student_lvl_4_ccna_discovery + nb_student_lvl_4_ccna_discovery

class CurrentExploration():

    #Local variables
    nb_student_lvl_1_ccna_exploration=0
    nb_student_lvl_2_ccna_exploration=0
    nb_student_lvl_3_ccna_exploration=0
    nb_student_lvl_4_ccna_exploration=0

    def __init__(self, nb_student_lvl_1_ccna_exploration, nb_student_lvl_2_ccna_exploration, nb_student_lvl_3_ccna_exploration, nb_student_lvl_4_ccna_exploration) :
        self.nb_student_lvl_1_ccna_exploration=nb_student_lvl_1_ccna_exploration
        self.nb_student_lvl_2_ccna_exploration=nb_student_lvl_2_ccna_exploration
        self.nb_student_lvl_3_ccna_exploration=nb_student_lvl_3_ccna_exploration
        self.nb_student_lvl_4_ccna_exploration=nb_student_lvl_4_ccna_exploration


    def add_som_exploration(self, nb_student_lvl_1_ccna_exploration, nb_student_lvl_2_ccna_exploration, nb_student_lvl_3_ccna_exploration, nb_student_lvl_4_ccna_exploration):
        self.nb_student_lvl_1_ccna_exploration=self.nb_student_lvl_1_ccna_exploration + nb_student_lvl_1_ccna_exploration
        self.nb_student_lvl_2_ccna_exploration=self.nb_student_lvl_2_ccna_exploration + nb_student_lvl_2_ccna_exploration
        self.nb_student_lvl_3_ccna_exploration=self.nb_student_lvl_3_ccna_exploration + nb_student_lvl_3_ccna_exploration
        self.nb_student_lvl_4_ccna_exploration=self.nb_student_lvl_4_ccna_exploration + nb_student_lvl_4_ccna_exploration

def en(chn):
    return chn.replace('Ž', 'e').replace('™', 'o').replace('è', 'e').replace('ö', 'o').replace('-', ' ').replace('´', ' ').replace("'", "e").replace('_', ' ').replace('ˆ','')

@app.route("/.api/netacad/rank_institution",  methods=['GET'])
@support_jsonp_rank_institution
def table_rank_institutions():
    ...
    try:
        ...
        for row in data :
            ...
            struct.append([str(info_institution[0]), str(info_institution[1]), str(info_institution[2]), str(row[1])])
...
@support_jsonp_switching_routing
...
def geolocation():
    ...
    try:
        ...
        cur.execute('SELECT name_institution, addr1_institution, addr2_institution, city_institution, zip_institution FROM institution')
        row = cur.fetchall()
        for line in row:
            #if city exist
            if line[3] != "":
...
@support_jsonp_geo_loc_institutions
        ...
        for row in data :
            if row[0] != "" :
                if row[0] == "Rhone-Alpes" :
                    ...
                    struct.append([str(row[0]), str(row[1])])
...
@support_jsonp_geo_loc_students
...
@support_jsonp_geo_click_region
...
@support_jsonp_curriculum_trending
...
@support_jsonp_split_curriculum_trending
